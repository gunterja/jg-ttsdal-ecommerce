...
class UserStoriesTest < ActionDispatch::IntegrationTest
  ...
  # User goes to index page. Selects a product, additing to their cart.
  # They then check out, filling in details on the checkout form. An order
  # is created with that info, plus a single line item with their 
  # selected product.
  test "buying a product" do
    ...
    order = Order.first
    assert_equal "Jeremy", order.name
    assert_equal "4113 Pineridge Dr.", order.address
    assert_equal "gunterja@gmail.com", order.email
    ...
    assert_equal ["gunterja@gmail.com"], mail.to
  end
end
